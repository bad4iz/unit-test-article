...
тут надо отредактировать будет

начали писать юнит тесты и столкнулись с тем что писать тесты стало сложнее и почти невозможно так как компонент огромный и логика в нем очень сложная и вложенная. и как это все покрыть тестами и не запутаться в моках. На это ушло много времени и усилий. Но пришло осознаие что тесты должны быть простыми и лаконичными и не должны быть сложными и вложенными. Стали думать и гадать что же в нас не так... оказалось что просто всю логику компонента-функции просто нельзя загрузить и распарсить у себя в голове там очень много всего... и тут дошло а как же SOLID ? а иммено Single Responsibility Principle (SRP)

> Single Responsibility Principle (SRP) - первая буква "S" в SOLID. SRP гласит, что каждый класс/функция должена иметь только одну причину для изменения, то есть одну ответственность.
>
...
и вот тут нас прорвало.. Оказывается что тесты пишутся быстро причем очень быстро. а фидбэк от них прилетает еще быстрее так как они выполняются молниеносно. Самая главная проблем заключается в том что сложный код не просто сложно поддерживать его архи сложно поддерживать даже писать юнит тесты для него ой как дорого. Вот тут да приходит на ум что черную коробочку лучше затестить интеграционными тестами и все это спрятать подальше под лавку и не трогать руками.

итого . если писать по уму как дядьки мудрые в старые времена без вайб кодинга говорили. то все то будет получатся и код поддерживаемый и тесты пишутся быстро и живется в проекте легко и текучки то оказывается можно избежать.
